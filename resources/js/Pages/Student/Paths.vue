  ...
  <StudentLayout>
    ...
    <div class="paths-grid">
      <div v-for="p in paths" :key="p.id" class="path-card">
        <div class="path-icon" :style="{ background: (p.color || '#38BDF8') + '22', color: p.color || '#38BDF8' }">
...
.page-head, .paths-grid, .modal-overlay {
  --sky:#38BDF8; --sky-dark:#0E7490; --pink:#EC4899; --pink-dark:#9D174D;
  --lime:#84CC16; --dark:#0F172A; --gray:#64748B; --border:#E2E8F0;
}
...
.btn.continue { background:var(--lime); color:#fff; box-shadow:0 4px 0 #3F6212; }
...
.modal-close { position:absolute; top:1rem; left:1rem; background:#F1F5F9; border:none; width:36px; height:36px; border-radius:50%; cursor:pointer; color:var(--gray); }
...
.grade-opt:hover { border-color:var(--sky); background:var(--sky-light, #E0F4FF); transform:translateY(-3px); }
