    ...
    <div v-else class="cert-grid">
      <div v-for="c in certificates" :key="c.id" class="cert-card">
        ...
        <a v-if="c.has_pdf" :href="route('student.certificates.download', c.id)" class="btn">
          <span class="mi">download</span> تحميل الشهادة
        </a>
      </div>
    </div>
  </StudentLayout>
...
.empty { text-align:center; color:#6B7280; font-weight:700; padding:3rem 1rem; }
...
.cert-card { background:#fff; border-radius:24px; padding:1.8rem 1.6rem; border:1.5px solid #f0eef7; text-align:center;
...
.cert-ribbon { width:56px; height:56px; border-radius:50%; background:linear-gradient(135deg,#7c5cbf,#f0806a);
  display:flex; align-items:center; justify-content:center; margin:0 auto 1rem; }
.cert-ribbon .mi { font-size:1.8rem; color:#fff; }
.cert-card h3 { font-size:1.15rem; font-weight:800; color:#1e1b2e; margin-bottom:.5rem; }
.cert-num { font-size:.8rem; color:#94A3B8; font-weight:700; margin-bottom:.4rem; font-family:monospace; }
.cert-date { font-size:.82rem; color:#6B7280; font-weight:700; display:flex; align-items:center; justify-content:center; gap:.3rem; margin-bottom:1.2rem; }
.cert-date .mi { font-size:1rem; }
.btn { display:inline-flex; align-items:center; gap:.4rem; padding:.7rem 1.6rem; border-radius:50px; background:#5f4398; color:#fff;
